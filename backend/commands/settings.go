		...
		},
		{
			...
			Options: []*discordgo.ApplicationCommandOption{
				...
				},
				{
					Type:        discordgo.ApplicationCommandOptionString,
					Name:        "snipe_limit",
					Description: "Maximum total time that can be added through anti-snipe (Example: 6h)",
					//Autocomplete: true,
				},
				{
					Type:        discordgo.ApplicationCommandOptionString,
...
func CurrencySetupClearButton(s *discordgo.Session, i *discordgo.InteractionCreate) error {
	return GenericSetupClear(s, i, &database.CurrencySetup{GuildID: i.GuildID}, SetupClearConfig{
		SetupType: "Currency",
		SetupCmd:  "/settings currency",
	})
}
